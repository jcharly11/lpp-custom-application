...
class MQTTService():
    ...
    def send_lpp_forbidden_tag_stats(self, epcs, http_code: int, elapsed_time: int, url: str, body: str):
        try:
            param = {
            ...
            topic = '{}/forbidden-tags/default'.format(
                settings.TOPIC_LPP_STATS)
            self.mqtt.publish(topic, param)
        except Exception as err:
            self.logger.error(
                f"send_lpp_forbidden_tag_stats {err}, {type(err)}")

    def send_lpp_forbidden_tag_error_stats(self, epcs, http_code: int, elapsed_time: int, url: str, error: str):
        try:
            param = {
                "timeStamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f"),
                "eventType": -1,
            ...
            self.logger.info(f"Sending forbidden tag stats {param}")
            topic = '{}/forbidden-tags/errors'.format(settings.TOPIC_LPP_STATS)
            self.mqtt.publish(topic, param)
        except Exception as err:
            self.logger.error(
                f"send_lpp_forbidden_tag_error_stats {err}, {type(err)}")

    def send_lpp_user_login_stats(self, http_code: int, elapsed_time: int, url: str):
        try:
            param = {
                "timeStamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f"),
                "eventType": 0,
            ...
            topic = '{}/user-login/default'.format(settings.TOPIC_LPP_STATS)
            self.mqtt.publish(topic, param)
        except Exception as err:
            self.logger.error(f"send_lpp_user_login_stats {err}, {type(err)}")

    def send_lpp_user_login_error_stats(self, http_code: int, elapsed_time: int, url: str, error: str):
        try:
            param = {
                "timeStamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f"),
                "eventType": -1,
            ...
            self.logger.info(f"Sending user login stats {param}")
            topic = '{}/user-login/errors'.format(settings.TOPIC_LPP_STATS)
            self.mqtt.publish(topic, param)
        except Exception as err:
            self.logger.error(
                f"send_lpp_user_login_error_stats {err}, {type(err)}")
