...
class LPPService():
    ...
    def authenticate(self, username, password):
        elapsed_time_ms = 0
        url = settings.LPP_URL_POST_LOGIN
        with httpx.Client() as client:
            try:
                ...
                self.mqtt_service.send_lpp_user_login_stats(
                ...
                # send to MQTT for monitoring
                self.mqtt_service.send_lpp_user_login_error_stats(
    ...
    def forbidden_tags(self, epcs, deviceType, deviceId, locationId):
        ...
        error = True
        elapsed_time_ms = 0
        url = settings.LPP_URL_FORBIDDEN_TAGS

        if self.token is None:
            self.logger.error(f"Authorization token is not yet available")
            # send to MQTT for monitoring
            self.mqtt_service.send_lpp_forbidden_tag_error_stats(
        ...
        with httpx.Client() as client:
            try:
                ...
                self.logger.info(
                    f"Response total milliseconds: {elapsed_time_ms}")
                # send to MQTT for monitoring
                self.mqtt_service.send_lpp_forbidden_tag_stats(
